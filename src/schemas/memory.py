...
class MemoryEntry(BaseModel):
    ...
    key: str = Field(description="记忆键名（建议使用 constants.memory.memory_description 中定义的键）")
    value: str = Field(description="记忆值（纯文本字符串，列表类型使用逗号分隔）")
    description: str | None = Field(default=None, description="键的描述（可从 constants.memory.memory_description 自动填充）")
...
class ChapterSummary(BaseModel):
    ...
    """
    chapter_index: int = Field(ge=0, description="章节索引（从0开始）")
    title: str = Field(description="章节标题")
    summary: str | None = Field(default=None, description="章节故事梗概（AI生成）")
